...
async function handleDriveOAuth(request: Request): Promise<Response | null> {
  ...
  if (url.pathname === "/api/drive/connect" && request.method === "GET") {
    ...
    const params = new URLSearchParams({ client_id: clientId, redirect_uri: GOOGLE_DRIVE_REDIRECT_URI, response_type: "code", access_type: "offline", prompt: "consent", include_granted_scopes: "true", scope: "https://www.googleapis.com/auth/drive", state });
  ...
  if (url.pathname === "/api/drive/callback" && request.method === "GET") {
    ...
    const tokenResponse = await fetch("https://oauth2.googleapis.com/token", { method: "POST", headers: { "content-type": "application/x-www-form-urlencoded" }, body: new URLSearchParams({ code, client_id: clientId, client_secret: clientSecret, redirect_uri: GOOGLE_DRIVE_REDIRECT_URI, grant_type: "authorization_code" }) });
