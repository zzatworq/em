...
const GOOGLE_DRIVE_REDIRECT_URI = "https://emonitor.zzatworq.dev/api/drive/callback";
...
async function handleDriveOAuth(request: Request): Promise<Response | null> {
  ...
  if (url.pathname === "/api/drive/callback" && request.method === "GET") {
    ...
    if (error) return Response.redirect("https://emonitor.zzatworq.dev/readings?drive=error", 302);
    ...
    return Response.redirect("https://emonitor.zzatworq.dev/readings?drive=connected", 302);
  }
