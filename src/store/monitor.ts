...
type State = MonitorData & { tab: TabId; selectedMonth: string | null; assume5050: boolean; hourlyDay: string; hourlyOverride: HourlyPoint[] | null; hydrated: boolean; dirty: boolean; setTab: (tab: TabId) => void; setMonth: (value: string) => void; setAssume5050: (v: boolean) => void; setHourlyDay: (v: string) => void; replaceData: (data: MonitorData) => void; markDirty: () => void; markSaved: () => void; addReading: (r: Omit<ReadingInput, "id">) => void; updateReading: (id: string, r: Partial<ReadingInput>) => void; deleteReading: (id: string) => void; clearAllReadings: () => void; addNote: (text: string) => void; deleteNote: (id: string) => void; addCollection: (c: Omit<Collection, "id">) => void; saveGeneral: (g: GeneralSettings) => void; saveTariffs: (t1: Tariff, t2: Tariff) => void; resetDemo: () => void };
...
export const useMonitor = create<State>()((set, get) => ({
  ...
  addCollection: (c) => { const collection = { ...c, id: uid("c") }; const collections = [...get().collections, collection]; set({ collections, history: syncCollectionHistory(get().history, collections), dirty: true }); },
