...
function ImageManager({ reading, onClose }: { reading: ReturnType<typeof useMonitor.getState>["readings"][number]; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4"><div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-elevated p-5 shadow-border">
    ...
    {storageOpen && <div className="mt-3 rounded-xl border border-border p-3"><p className="mb-2 text-sm font-medium">Unrelated / unattached images</p>{available.length ? <div className="grid gap-2 sm:grid-cols-2">{available.map((image) => <button key={image.id} type="button" className="flex items-center gap-3 rounded-lg border border-border p-2 text-left hover:bg-background" onClick={() => void attach(image)}><ImageThumb driveFileId={image.driveFileId!} alt={image.id} /><span className="text-xs">{new Date(image.imageCreatedAt ?? image.createdAt).toLocaleString()}</span></button>)}</div> : <p className="text-sm text-muted">No unattached images.</p>}</div>}
  </div></div>;
}
