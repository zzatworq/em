...
function TimeField({ label, hour, minute, onChange }: { label: string; hour: number; minute: number; onChange: (hour: number, minute: number) => void }) {
  ...
  return <div className="min-w-0"><Label>{label}</Label><Input className="mt-1 min-w-0 w-full" type="time" value={`${pad(hour)}:${pad(minute)}`} onChange={(e) => { const [h, m] = e.target.value.split(":").map(Number); if (!Number.isNaN(h) && !Number.isNaN(m)) onChange(h, m); }} /></div>;
}
  ...
  return (
    <div className="space-y-5">
      <section className="rounded-2xl bg-elevated p-4 shadow-border sm:p-6">
        <div className="grid min-w-0 gap-6 md:grid-cols-[11rem_minmax(0,1fr)] lg:grid-cols-[13rem_minmax(0,1fr)]">
          <aside className="min-w-0">
            <div className="rounded-xl border border-border bg-background/40 p-2" role="tablist" aria-label="Settings sections">
              ...
              <div className="grid min-w-0 gap-1">
                {tabs.map(([id, label]) => <button key={id} type="button" role="tab" aria-selected={tab === id} onClick={() => setTab(id)} className={`min-w-0 w-full rounded-lg px-3 py-2.5 text-left text-sm font-medium transition-colors ${tab === id ? "bg-foreground text-background" : "text-muted hover:bg-elevated hover:text-foreground"}`}>{label}</button>)}
              </div>
            </div>
          </aside>
          <div className="min-w-0 w-full">
            ...
            {tab === "appearance" && <div>
              ...
              <div className="mt-5 grid min-w-0 gap-3 sm:grid-cols-2 lg:grid-cols-3"><div className="min-w-0"><Label>Theme</Label><select className="mt-1 h-10 w-full min-w-0 rounded-md border border-border bg-background px-3 text-sm" value={g.theme} onChange={(e) => setG({ ...g, theme: e.target.value as GeneralSettings["theme"] })}><option value="system">System</option><option value="light">Light</option><option value="dark">Dark</option></select></div><div className="min-w-0"><Label>Meter 1 colour</Label><div className="mt-1 flex min-w-0 gap-2"><input className="h-10 w-12 shrink-0 cursor-pointer rounded-md border border-border bg-background p-1" type="color" value={g.meter1Color} onChange={(e) => setG({ ...g, meter1Color: e.target.value })} /><Input className="min-w-0 w-full" value={g.meter1Color} onChange={(e) => setG({ ...g, meter1Color: e.target.value })} /></div></div><div className="min-w-0"><Label>Meter 2 colour</Label><div className="mt-1 flex min-w-0 gap-2"><input className="h-10 w-12 shrink-0 cursor-pointer rounded-md border border-border bg-background p-1" type="color" value={g.meter2Color} onChange={(e) => setG({ ...g, meter2Color: e.target.value })} /><Input className="min-w-0 w-full" value={g.meter2Color} onChange={(e) => setG({ ...g, meter2Color: e.target.value })} /></div></div><div className="min-w-0 sm:col-span-2 lg:col-span-3"><Field label="V1 app URL" value={g.v1Url} onChange={(v) => setG({ ...g, v1Url: v })} /></div></div>
            ...
            {tab === "tariffs" && <div>
              ...
              <div className="mt-5 min-w-0 space-y-3"><TariffEditor title="Meter 1" value={a} onChange={setA} /><TariffEditor title="Meter 2" value={b} onChange={setB} /></div>
