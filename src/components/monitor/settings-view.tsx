...
export function SettingsView() {
  ...
  async function handleImport(file: File) {
    try {
      ...
      replaceData(backup.data); markDirty();
      setG(backup.data.general); setA(backup.data.tariff1); setB(backup.data.tariff2);
  ...
  return (
    <div className="space-y-5">
      <section className="rounded-2xl bg-elevated p-5 shadow-border sm:p-6">
        ...
        {tab === "data" && <div>
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            <div className="rounded-xl border border-border p-4">
              ...
              <p className="mt-1 text-sm text-muted">Readings, collections, history, notes, settings and tariffs.</p>
              <div className="mt-3 flex flex-wrap gap-2"><Button onClick={exportAll}>Export all data</Button><Button variant="outline" onClick={() => fileRef.current?.click()}>Import backup</Button></div>
            </div>
            <div className="rounded-xl border border-border p-4">
              <p className="font-medium">Readings CSV</p>
              <p className="mt-1 text-sm text-muted">Exchange readings with Excel or Google Sheets. Duplicate rows are skipped.</p>
              <div className="mt-3 flex flex-wrap gap-2"><Button variant="outline" onClick={exportCsv}>Export readings CSV</Button><Button variant="outline" onClick={() => fileRef.current?.click()}>Import readings CSV</Button></div>
            </div>
          </div>
        ...
        {tab === "general" && <div className="pt-5">
        ...
        {tab === "appearance" && <div className="pt-5">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            <div><Label>Theme</Label><select className="mt-1 h-10 w-full rounded-md border border-border bg-background px-3 text-sm" value={g.theme} onChange={(e) => setG({ ...g, theme: e.target.value as GeneralSettings["theme"] })}><option value="system">System</option><option value="light">Light</option><option value="dark">Dark</option></select></div>
        ...
        {tab === "tariffs" && <div className="pt-5">
          <h2 className="font-display text-2xl font-medium">Meter tariffs</h2>
          <p className="mt-1 text-sm text-muted">MEPCO A-1 residential slabs. Each meter remains independently configured.</p>
