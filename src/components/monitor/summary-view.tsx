...
function formatGraphTime(hour: number, minute: number) {
...
function HistoricalProfileChart({ profile }: { profile: HourlyProfilePoint[] }) {
  ...
  useEffect(() => { setColor(themeColor("--color-profile", "#E3B341")); }, []);
  const data = profile.map((p) => ({ label: formatGraphTime(p.hour, p.minute), percent: p.percent }));
...
export function SummaryView() {
  const { dashboard, hourlyProfile } = useDashboard();
  ...
  const displayedUsage = Number(d.billingTotal || 0);
  const carryForward = Number(d.carryForwardNew || 0) + Number(d.carryForwardOld || 0);
  const projectedUsage = d.isCurrentBillingMonth ? Number(d.projectedTotal || 0) : displayedUsage;
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-2xl bg-elevated p-5 shadow-border sm:p-6"><div className="mb-3 flex items-center justify-between gap-3"><h3 className="font-display text-lg font-medium">Meter split</h3><div className="flex items-center gap-3 text-xs text-muted"><span>Meter 1 · {units(d.billingNew)} kWh</span><span>Meter 2 · {units(d.billingOld)} kWh</span></div></div><div className="flex h-3 overflow-hidden rounded-full bg-black/25"><div className="bg-meter1" style={{ width: `${Number(d.meter1Share || 0)}%` }} /><div className="bg-meter2" style={{ width: `${Number(d.meter2Share || 0)}%` }} /></div><div className="mt-2 flex justify-between text-xs text-muted"><span>{Number(d.meter1Share || 0).toFixed(0)}%</span><span>{Number(d.meter2Share || 0).toFixed(0)}%</span></div></section>
    {d.isCurrentBillingMonth ? <section className="rounded-2xl bg-elevated p-5 shadow-border sm:p-6"><p className="text-xs font-medium uppercase tracking-wider text-muted">Estimated projection</p><div className="mt-2 flex flex-wrap items-end justify-between gap-4"><div><p className="font-display text-3xl font-medium tabular-nums">Projected month {units(projectedUsage)} kWh</p><p className="mt-1 text-sm text-muted">{units(displayedUsage)} used + {units(remainingFuture)} future estimate</p></div><div className="text-right text-sm tabular-nums text-muted"><p>Last 15 completed days: {units(d.projection.rolling15Average)} kWh/d</p><p>{units(d.projection.rolling15Days, 0)} days used for average</p><p>{units(d.projection.remainingDays, 1)} days remaining</p></div></div><div className="mt-4 grid gap-3 border-t border-border pt-4 sm:grid-cols-3"><div><p className="text-xs text-muted">Future estimate</p><p className="font-display text-xl tabular-nums">{units(remainingFuture)} kWh</p></div><div><p className="text-xs text-muted">Projected bill</p><p className="font-display text-xl tabular-nums">{money(displayedBill)}</p></div><div><p className="text-xs text-muted">Effective rate</p><p className="font-display text-xl tabular-nums">Rs {effectiveRate.toFixed(2)}/kWh</p></div></div></section> : null}
    ...
    {hourlyProfile?.length ? <section className="rounded-2xl bg-elevated p-5 shadow-border sm:p-6"><div className="mb-3"><h3 className="font-display text-lg font-medium">Historical consumption profile</h3><p className="mt-1 text-xs text-subtle">Hourly distribution across all completed days, weighted by the number of readings recorded on each day.</p></div><div className="mb-2 inline-flex items-center gap-1.5 text-xs text-muted"><span className="h-0.5 w-4 rounded bg-profile" /> Historical profile</div><HistoricalProfileChart profile={hourlyProfile} /></section> : null}
  </div>;
}
